...
namespace NextIteration.SpectreConsole.Auth.Tests.Commands
{
    public sealed class CommandFormattingTests
    {
        ...
        // Regression: previously every site sliced AccountId[..8] which throws
        // ArgumentOutOfRangeException for short user-supplied ids (e.g.
        // `accounts delete abc`).
        public void ShortId_ShortString_ReturnsFullStringWithoutThrowing(string id) =>
            Assert.Equal(id, CommandFormatting.ShortId(id));

        [Fact]
        public void ShortId_ExactlyEight_ReturnsValuePlusEllipsis() => Assert.Equal("12345678...", CommandFormatting.ShortId("12345678"));

        [Theory]
        [InlineData(null)]
        [InlineData("")]
        public void ShortId_NullOrEmpty_ReturnsEmpty(string? id) => Assert.Equal(string.Empty, CommandFormatting.ShortId(id));
    }
}
